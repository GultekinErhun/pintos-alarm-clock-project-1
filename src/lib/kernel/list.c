...
// Insert in timer_wait_list in a sorted way (ascending order). By Shikhar
void
list_insert_sorted(struct list *list, struct list_elem *e_to_place)
{

  struct list_elem *e;

  list_push_back(list, e_to_place);
  //printf("Push Successful !\n");
  // for (e = list_begin (list); e != list_end (list); e = list_next (e)) {
  //     t1 = list_entry(e, struct thread, allelem);
  //     t2 = list_entry(e_to_place, struct thread, allelem);
  //     // Compare
  //     if(t1->time_wakeup > t2->time_wakeup)
  //       break;
  //     else
  //       continue;
  // }
